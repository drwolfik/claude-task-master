...
function parseSubtasksFromText(
	...
	logger.debug(`Starting parseSubtasksFromText with startId=${startId}, expectedCount=${expectedCount}, parentTaskId=${parentTaskId}`);
	...
	const hasJsonStructure = jsonToParse.includes('"subtasks"') || 
		jsonToParse.includes('```json') ||
		(jsonToParse.trim().startsWith('{') && jsonToParse.trim().endsWith('}'));
	
	logger.debug(`JSON structure check: hasJsonStructure=${hasJsonStructure}, contains subtasks: ${jsonToParse.includes('"subtasks"')}`);
	
	if (!hasJsonStructure) {
		logger.debug('Response appears to be plain text from Claude Code CLI. Attempting to extract subtasks from text format.');
		
		// Try to extract subtasks from plain text format
		const extractedSubtasks = extractSubtasksFromPlainText(jsonToParse, startId, expectedCount, parentTaskId, logger);
		if (extractedSubtasks && extractedSubtasks.length > 0) {
			logger.warn(`SUCCESS: Successfully extracted ${extractedSubtasks.length} subtasks from plain text response.`);
			return extractedSubtasks;
		} else {
			logger.debug('Failed to extract subtasks from plain text. Falling back to JSON parsing.');
		}
	} else {
	...
	if (primaryParseAttemptFailed || !parsedObject) {
		...
		if (patternStartIndex !== -1) {
			...
			if (openBraces === 0 && extractedJsonBlock.length > 0) {
				...
				logger.debug('Advanced extraction: Successfully extracted JSON block using target pattern.');
			} else {
				logger.debug('Advanced extraction: Target pattern found but brace counting failed. Trying fallback extraction.');
				...
				if (lastOpenBrace !== -1 && lastCloseBrace !== -1 && lastCloseBrace > lastOpenBrace) {
					...
					logger.debug('Advanced extraction: Using fallback extraction (last complete JSON object).');
				}
			}
		} else {
			logger.debug('Advanced extraction: Target pattern not found. Trying fallback extraction.');
			// Fallback: try to find the last complete JSON object
			const lastOpenBrace = jsonToParse.lastIndexOf('{');
			const lastCloseBrace = jsonToParse.lastIndexOf('}');
		...
		try {
			...
			logger.debug(`Original response for text extraction: ${originalTrimmedResponse.substring(0, 500)}...`);
			
			try {
				const extractedSubtasks = extractSubtasksFromPlainText(originalTrimmedResponse, startId, expectedCount, parentTaskId, logger);
				if (extractedSubtasks && extractedSubtasks.length > 0) {
					logger.info(`SUCCESS: Extracted ${extractedSubtasks.length} subtasks from plain text response.`);
					return extractedSubtasks;
				} else {
					logger.debug('Plain text extraction returned empty array.');
				}
			} catch (extractError) {
				logger.error(`Error in extractSubtasksFromPlainText: ${extractError.message}`);
			}
			
			// --- NEW: Enhanced fallback for mixed text+JSON responses ---
			logger.debug('JSON parsing failed. Attempting to extract JSON from mixed text+JSON response.');
			
			// Try to extract JSON from the original response (might contain text + JSON)
			const jsonMatch = originalTrimmedResponse.match(/\{[\s\S]*"subtasks"[\s\S]*\}/);
			if (jsonMatch) {
				try {
					const extractedJson = jsonMatch[0];
					logger.debug(`Found JSON block in mixed response: ${extractedJson.substring(0, 200)}...`);
					parsedObject = JSON.parse(extractedJson);
					logger.info('Successfully parsed JSON from mixed text+JSON response.');
				} catch (jsonParseError) {
					logger.warn(`Failed to parse extracted JSON: ${jsonParseError.message}`);
				}
			}
 ...
 */
function extractSubtasksFromPlainText(text, startId, expectedCount, parentTaskId, logger) {
	...
	for (const rawSubtask of subtasks) {
		...
		if (result.success) {
			validatedSubtasks.push(result.data);
					...
					logger.warn(`Plain text subtask validation failed: ${JSON.stringify(rawSubtask).substring(0, 100)}...`);
			logger.warn(`Plain text subtask validation failed: ${JSON.stringify(rawSubtask).substring(0, 100)}...`);
		}
	}
	
	logger.debug(`Extracted ${subtasks.length} raw subtasks, validated ${validatedSubtasks.length} subtasks`);
	...
	try {
		...
		try {
			...
			if (error.message && error.message.includes('Claude Code process exited with code')) {
				logger.error(`Claude Code CLI error detected for task ${taskId}. This may be due to Ink interface issues on Windows.`);
				logger.error(`Suggested solutions: Use PowerShell instead of Git Bash, or set environment variables FORCE_COLOR=0 CI=true`);
			}
			
			// Special handling for instanceof errors
			if (error.message && error.message.includes('instanceof')) {
				logger.error(`Type checking error detected for task ${taskId}. This may be due to undefined classes or modules not being properly loaded.`);
			}
		...
		if (error.message && error.message.includes('Claude Code process exited with code')) {
			logger.error(`Claude Code CLI error in expand-task for task ${taskId}. This is a known issue on Windows with Git Bash.`);
			logger.error(`Solutions: Use PowerShell, set FORCE_COLOR=0 CI=true, or try a different AI provider.`);
		}
		
		if (error.message && error.message.includes('instanceof')) {
			logger.error(`Type checking error in expand-task for task ${taskId}. This may indicate a module loading issue.`);
		}
		
		if (error.message && error.message.includes('not found')) {
			logger.error(`Task or file not found error in expand-task for task ${taskId}. Check if the task exists and file paths are correct.`);
		}
